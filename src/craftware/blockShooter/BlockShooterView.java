...
public class BlockShooterView extends SurfaceView implements
	...
	class BlockShooterThread extends Thread implements OnJetEventListener {
		...
		// array to store the mute masks that are applied during game play to
		// respond to
		...
		public BlockShooterThread(SurfaceHolder surfaceHolder, Context context,
			...
			// JET info: this are the mute arrays associated with the music beds
			// in the
			// JET info: JET file
		...
		private void initializeJetPlayer() {
			...
			// JET info: otherwise left over events from previous gameplay can
			// hang around.
			// JET info: ok, here we don't really need that but if you ever
			// reuse a JetPlayer
			// JET info: instance, clear the queue before reusing it, this will
			// also clear any
			// JET info: trigger clips that have been triggered but not played
			// yet.
			mJet.clearQueue();

			// JET info: we are going to receive in this example all the JET
			// callbacks
			...
			// JET info: it's stored as a raw resource in our APK, and is
			// labeled "level1"
			mJet.loadJetFile(mContext.getResources().openRawResourceFd(
					R.raw.level1));
			// JET info: if our JET file was stored on the sdcard for instance,
			// we would have used
			...
			// JET info: now we're all set to prepare queuing the JET audio
			// segments for the game.
			// JET info: in this example, the game uses segment 0 for the
			// duration of the game play,
			// JET info: and plays segment 1 several times as the "outro" music,
			// so we're going to
			// JET info: queue everything upfront, but with more complex JET
			...
			// JET info: it uses the first DLS lib in the .jet resource, which
			// is at index 0
			...
			// JET info: set the mute mask as designed for the beginning of the
			// game, when the
		...
		private void doDrawRunning(Canvas canvas) {
			...
			canvas.drawBitmap(mShipFlying[mShipIndex], mBlockShooterX,
					mBlockShooterY, null);

			if (mLaserOn) {
				canvas.drawBitmap(mLaserShot, mBlockShooterX
						+ mShipFlying[0].getWidth(), mBlockShooterY
						+ (mShipFlying[0].getHeight() / 2), null);
			}

			// tick tock
			canvas.drawBitmap(mTimerShell, mCanvasWidth
		...
		private void doAsteroidAnimation(Canvas canvas) {
			...
			for (int i = (mDangerWillRobinson.size() - 1); i >= 0; i--) {
				Block asteroid = mDangerWillRobinson.elementAt(i);

				if (!asteroid.mMissed)
					mBlockShooterY = asteroid.mDrawY;

				// Log.d(TAG, " drawing asteroid " + ii + " at " +
				// asteroid.mDrawX );

				canvas.drawBitmap(mAsteroids[(asteroid.mAniIndex + animOffset)
						% mAsteroids.length], asteroid.mDrawX, asteroid.mDrawY,
						null);
			}

			for (int i = (mExplosion.size() - 1); i >= 0; i--) {
				Particle ex = mExplosion.elementAt(i);

				canvas.drawBitmap(mExplosions[(ex.mAniIndex + animOffset)
			...
			while (true) {
				...
				// JET events trigger a state update
					...
					processJetEvent(jetEvent.player, jetEvent.segment,
							jetEvent.track, jetEvent.channel,
							jetEvent.controller, jetEvent.value);
				}
			}
		}
		...
		protected void updateLaser(Object inputContext) {
			...
			long keyTime = inputContext == null ? 0
					: ((GameEvent) inputContext).eventTime;
			...
			// trying to tune the laser hit timing
				...
				// JET info: the laser sound is on track 23, we mute it (true)
				// right away (false)
		...
		protected void updateAsteroids(Object inputContext) {
			...
			for (int i = (mDangerWillRobinson.size() - 1); i >= 0; i--) {
				...
				// check if the key was pressed close enough to the beat to make
				// a hit
				if (asteroid.mDrawX <= mAsteroidMoveLimitX + 20
						&& !asteroid.mMissed) {
					...
					if (mLaserOn) {
						...
						// as missed so on next frame it cannot be hit even if
						// it is still
						// within range
		 ...
		 * events.
		 */
		protected void processJetEvent(JetPlayer player, short segment,
				byte track, byte channel, byte controller, byte value) {

			// Log.d(TAG, "onJetEvent(): seg=" + segment + " track=" + track +
			// " chan=" + channel
			...
			// if the intensity has go gone up, call a corresponding trigger
			// clip, otherwise just
			...
			if (mBeatCount == 1) {
				...
				if (mHitStreak > 28) {
					...
					if (mCurrentBed != 7) {
						...
						mCurrentBed = 7;
						// JET info: change the mute mask to update the way the
						// music plays based
					...
					if (mCurrentBed != 6) {
						if (mCurrentBed < 6) {
							// JET info: quite a few asteroids hit, trigger the
							// clip with the guy's
		...
		private void doCountDown() {
			...
			// this is how we send data back up to the main BlockShooterView
			// thread.
			// if you look in constructor of BlockShooterView you will see code
			// for
			// Handling of messages. This is borrowed directly from lunar
			// lander.
			// Thanks again!
			msg.setData(b);
		 ...
		 * where we queue up events 80 and 82.
		 * 
		 * Most of this data passed is unneeded for BlockShooter logic but shown
		...
		public void onJetUserIdUpdate(JetPlayer player, int userId,
				int repeatCount) {
			// Log.i(TAG, "onJetUserIdUpdate(): userId =" + userId +
	...
	public BlockShooterView(Context context, AttributeSet attrs) {
		...
		if (isInEditMode() == false) {
			thread = new BlockShooterThread(holder, context, new Handler() {
